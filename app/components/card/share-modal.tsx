...
import { ChevronLeft, X, ExternalLink, Facebook, Twitter, Linkedin, MessageCircle } from "lucide-react";
import {
  Dialog,
  DialogContent,
              ...
              <div className="flex justify-center gap-4">
                {company?.facebook_url && (
                  <button
                    ...
                    className="w-12 h-12 rounded-full bg-white border-2 border-gray-200 flex items-center justify-center hover:bg-gray-50 transition-colors"
                    title="Share on Facebook"
                  >
                    <Facebook className="h-6 w-6 text-gray-700" />
                  </button>
                )}
                {(company?.linkedin_url || card.social_links?.linkedin) && (
                  <>
                    <button
